"""Exploración estadística de la cohorte COMA (outputs/nb09_coma). Solo gráficos en pantalla."""
...
def plot_cohort_overview(cohort):
    ...
    fig.suptitle("COMA — composición de cohorte", y=1.02)
    plt.tight_layout()
    plt.show()
...
def plot_age_distributions(cohort):
    ...
    fig.suptitle("COMA — distribuciones de edad", y=1.02)
    plt.tight_layout()
    plt.show()
...
def plot_clinical(cohort):
    ...
    if has_out:
        ...
        axes[i].set_xlabel("")
    fig.suptitle("COMA — variables clínicas", y=1.02)
    plt.tight_layout()
    plt.show()
...
def plot_bag_mae(bag):
    ...
    fig.suptitle("COMA — brain age / BAG / error", y=1.02)
    plt.tight_layout()
    plt.show()
    ...
    ax.set_title("COMA — BAG vs edad cronológica")
    plt.tight_layout()
    plt.show()
...
def plot_topo(topo):
    ...
    fig.suptitle("COMA — métricas topológicas (TOPO)", y=1.01)
    plt.tight_layout()
    plt.show()
...
def plot_bag_vs_clinical(bag, cohort):
    ...
    fig.suptitle("COMA — BAG vs clínica", y=1.02)
    plt.tight_layout()
    plt.show()
...
def plot_summary_table(cohort, bag, topo):
    ...
    tbl.set_fontsize(9)
    tbl.scale(1.15, 1.4)
    ax.set_title("COMA — tabla resumen", pad=12)
    plt.tight_layout()
    plt.show()
...
def main():
    ...
    print(f"cohort={len(cohort)}  bag={len(bag)}  topo={len(topo)}  pred_coma={len(pred)}")

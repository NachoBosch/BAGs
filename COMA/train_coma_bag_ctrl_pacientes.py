"""
Pipeline COMA CTRL vs pacientes (ANOX+TRAU unidos).

Mismo flujo que train_coma_bag.py:
1. Inventario FC + demografía
2. Remap ANOX/TRAU → pacientes
3. TOPO (umbral 0.20)
4. β-VAE entrenado en FC de coma
5. Ridge(Z+TOPO+sex → edad) + BAG/MAE
6. Tablas en COMA/; VAE/embeddings en outputs/coma/
"""
...
VAE_DIR = OUT / "vae_coma_ctrl_pacientes"
...
def step_cohort() -> pd.DataFrame:
    ...
    cohort.to_csv(TABLE_DIR / "cohort_coma_ctrl_pacientes.csv", index=False)
...
def step_topo(cohort: pd.DataFrame) -> pd.DataFrame:
    ...
    topo.to_csv(TABLE_DIR / "graph_metrics_coma_ctrl_pacientes.csv", index=False)
...
def step_vae(cohort: pd.DataFrame) -> tuple[object, np.ndarray, list[str]]:
    ...
    if REUSE_VAE and (VAE_DIR / "vae.weights.h5").exists():
        ...
        print("entrenando VAE CTRL vs pacientes…")
        vae, hist = train_vae_final(X, VAE_DIR, seed=SEED, **VAE_HP)
        with open(OUT / "vae_history_summary_ctrl_pacientes.json", "w") as f:
            json.dump({k: [float(x) for x in v] for k, v in hist.items()}, f)

    Z = encode_mu(vae.encoder, X)
    save_embeddings(OUT / "embeddings_mu_ctrl_pacientes", ids, Z)
    print("Z:", Z.shape)
    return vae, Z, ids
...
def step_ridge_bag(
    ...
    extra_cols = ["sub_code", "patient", "sex", "gcs", "binary_outcome", "etiology"]
    if "diagnosis_orig" in cohort.columns:
        extra_cols.append("diagnosis_orig")
    extra = cohort.set_index("record_id")[extra_cols]
    bag_df = bag_df.join(extra, on="record_id")

    bag_path = TABLE_DIR / "bag_coma_ctrl_pacientes.csv"
    ...
    with open(TABLE_DIR / "metrics_coma_ctrl_pacientes.json", "w") as f:
...
def main():
    ...
    print("OUT:", OUT)
    print("contraste: CTRL vs", PATIENT_LABEL, f"(fuentes={sorted(PATIENT_SOURCES)})")

...
def load_coma():
    cohort = pd.read_csv(DATA / f"cohort_coma_ctrl_pacientes{SUFFIX}.csv")
    bag = pd.read_csv(DATA / f"bag_coma_ctrl_pacientes{SUFFIX}.csv")
    topo = pd.read_csv(DATA / f"graph_metrics_coma_ctrl_pacientes{SUFFIX}.csv")

    cohort = cohort[cohort["diagnosis"].isin(KEEP)].copy()
    bag = bag[bag["diagnosis"].isin(KEEP)].copy()
    topo = topo[topo["diagnosis"].isin(KEEP)].copy()

    cohort["sex"] = cohort["sex"].replace({
        "H": "M", "Male": "M", "MALE": "M",
        "Female": "F", "FEMALE": "F",
    })
...
def plot_cohort_overview(cohort):
    order = _order(cohort)
    n_panels = 4 if "source" in cohort.columns else 3
    ...
    if "source" in cohort.columns:
        src = pd.crosstab(cohort["diagnosis"], cohort["source"]).reindex(order)
        src.plot(kind="bar", ax=axes[3], rot=0)
        axes[3].set_title("origen × grupo")
        axes[3].set_xlabel("")
    ...
    if "source" in cohort.columns:
        print("\nsource × diagnosis:")
        print(pd.crosstab(cohort["diagnosis"], cohort["source"]))
...
def plot_summary_table(cohort, bag, topo):
    ...
    for dx in order:
        ...
        if "source" in c.columns and dx == "CTRL":
            row["n_coma"] = int((c["source"] == "coma").sum())
            row["n_redlat"] = int((c["source"] == "redlat").sum())
        rows.append(row)
